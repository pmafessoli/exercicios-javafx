...
public class Wizard extends Application {
	...
	private void criarPasso2() {
		...
		Button retornarPasso = new Button("<<Voltar p/ Passo1");
		retornarPasso.setOnAction(e -> {
			janela.setScene(passo1);
		...
		HBox box = new HBox();
		box.setAlignment(Pos.CENTER);
		box.getChildren().add(retornarPasso);
		box.getChildren().add(proximoPasso);
		passo2 = new Scene(box, 400, 400);
	}
	
	private void criarPasso3() {
		Button proximoPasso = new Button("Fchar");
		proximoPasso.setOnAction(e -> {
			janela.close();
		});
		Button retornarPasso = new Button("<<Voltar p/ Passo2");
		retornarPasso.setOnAction(e -> {
			janela.setScene(passo2);
